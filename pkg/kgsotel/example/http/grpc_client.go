...
import (
	...
	"errors"
	"fmt"
	kgsotel "hype-casino-platform/pkg/kgsotel"
...
func callSayHello(ctx context.Context, c api.HelloServiceClient) error {
	...
	kgsotel.Info(ctx, "Response from server", kgsotel.NewField("reply", response.Reply))
	return nil
}
...
func callSayHelloBidiStream(ctx context.Context, c api.HelloServiceClient) error {
	...
	go func() {
		...
		if err != nil {
			kgsotel.Error(ctx, "closing SayHelloBidiStream", kgsotel.NewField("error", err))
		}
	...
	go func() {
		for {
			...
			if errors.Is(err, io.EOF) {
				...
				kgsotel.Error(ctx, "receiving from SayHelloBidiStream", kgsotel.NewField("error", err))
			}
