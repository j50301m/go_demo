...
import (
	...
	"hype-casino-platform/pkg/kgsotel/example/api"
	"io"
...
func (s *server) SayHello(ctx context.Context, in *api.HelloRequest) (*api.HelloResponse, error) {
	...
	kgsotel.Info(ctx, "SayHello", kgsotel.NewField("greeting", in.Greeting))
...
func (s *server) doSomething(ctx context.Context) {
	...
	// Simulate some work
	time.Sleep(50 * time.Millisecond)
	kgsotel.Info(ctx, "doSomething", kgsotel.NewField("key", "value"))
}
...
func (s *server) SayHelloClientStream(stream api.HelloService_SayHelloClientStreamServer) error {
	...
	for {
		...
		if errors.Is(err, io.EOF) {
			...
			kgsotel.Error(ctx, "SayHelloClientStream", kgsotel.NewField("error", err))
			return err
		}

		kgsotel.Info(ctx, "SayHelloClientStream", kgsotel.NewField("greeting", in.Greeting))
		i++
	}
...
func (s *server) SayHelloBidiStream(stream api.HelloService_SayHelloBidiStreamServer) error {
	...
	for {
		...
		if errors.Is(err, io.EOF) {
			...
			kgsotel.Error(ctx, "SayHelloBidiStream", kgsotel.NewField("error", err))
			return err
		}

		time.Sleep(50 * time.Millisecond)

		kgsotel.Info(ctx, "SayHelloBidiStream", kgsotel.NewField("greeting", in.Greeting))
